...
func SendResponseInterface(data *Data, reply interface{}, escapeEveryoneMention bool) ([]*discordgo.Message, error) {
	...
	case string:
		if t != "" {
			...
			return dutil.SplitSendMessage(data.Session, data.CS.ID(), t)
		}
	...
	case error:
		if t != nil {
			...
			return dutil.SplitSendMessage(data.Session, data.CS.ID(), m)
		}
		return []*discordgo.Message{}, nil
	case *discordgo.MessageEmbed:
		m, err := data.Session.ChannelMessageSendEmbed(data.CS.ID(), t)
		return []*discordgo.Message{m}, err
	case []*discordgo.MessageEmbed:
		msgs := make([]*discordgo.Message, len(t))
		for i, embed := range t {
			m, err := data.Session.ChannelMessageSendEmbed(data.CS.ID(), embed)
			if err != nil {
				return msgs, err
			}
			msgs[i] = m
...
func (t *TemporaryResponse) Send(data *Data) ([]*discordgo.Message, error) {
	...
	time.AfterFunc(t.Duration, func() {
		...
		if len(msgs) > 1 {
			...
			data.Session.ChannelMessagesBulkDelete(data.CS.ID(), ids)
		} else {
			data.Session.ChannelMessageDelete(data.CS.ID(), msgs[0].ID)
		}
	})
	return msgs, nil
}
...
func (fe *FallbackEmebd) Send(data *Data) ([]*discordgo.Message, error) {
	channelPerms, err := data.Session.State.UserChannelPermissions(data.Session.State.User.ID, data.CS.ID())
	if err != nil {
		return nil, err
	}

	if channelPerms&discordgo.PermissionEmbedLinks != 0 {
		m, err := data.Session.ChannelMessageSendEmbed(data.CS.ID(), fe.MessageEmbed)
		if err != nil {
			return nil, err
		}
	...
	return dutil.SplitSendMessage(data.Session, data.CS.ID(), content)
}
